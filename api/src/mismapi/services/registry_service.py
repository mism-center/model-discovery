...
class RegistryService:
    ...
    def write_metadata_package_raw(
        ...
        *,
        ...
        legitimately be missing data the annotator couldn't confidently
        extract; approval must not be blocked by that. If the parsed package
        can't be fully mapped onto a Resource (whether individual fields are
        missing — handled by ``build_resource_from_package`` itself — or the
        package fails to parse at all), the model is still approved: derived
        fields are simply left as previously stored, and the issue is
        returned as a warning instead of raising.

        Raises 403 (not owner), 404 (missing), 400 (unknown filename or
        syntactically malformed YAML — a genuine editing mistake, not
        missing annotator data).
        """
        ...
        # Approval must not be blocked by validation issues in annotator-generated
        # content the user can't edit — a parse failure is downgraded to a warning
        # rather than raised, and previously stored derived fields are preserved.
        warnings: list[str] = []
        parsed: Resource | None = None
        try:
            parsed, parse_warnings = build_resource_from_package(pkg_dir)
            ...
            logger.warning(
                "Metadata-package for model %s could not be fully parsed; approving "
                "without updating derived fields: %s",
                model_id,
                exc,
            )
            warnings.append(
                f"Metadata-package could not be fully parsed ({exc}); the model was "
                "still approved and previously stored fields were preserved."
            )
        ...
        # Apply YAML-derived fields (skipped if parsing failed above; the resource
        # keeps its previously stored fields in that case). Always preserve
        # system-managed fields regardless: id, owner, registration_status, metadata
        # dict, location_uri (iRODS path), execution_ref, format_tags, digest_sha256,
        ...
        # Approval is never blocked by metadata-package validation issues — the
        # registration status is always advanced here regardless of `parsed`.
