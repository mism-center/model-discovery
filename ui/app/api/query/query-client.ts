 ...
 * - On a 401 from a non-auth query, drop the cached user to `null` and
 *    capabilities to all-false so the UI flips to a signed-out state.
 *    403s are permission denials, not session expiry — they must not clear
 *    the session. Both auth queries already resolve those same values on 401
 *    themselves (`fetchUser`, `fetchCapabilities`), so the
